...
const onChangeHandler = (onChange, type) => (e) => {
  if (type === 'date' || type === 'datepicker' || type === 'datetime' || type ==='time' || type === 'timepicker') { return onChange(e);}
...
export default ({ schema = {}, uiSchema = {}, onChange, onBlur, onFile, htmlId, data, objectData }) => {
  ...
  if(schema.check === 'onblur'){
    rv.onBlur = onBlur;
  }
  if(schema.content === 'file') rv.onFile = onFile;
  return rv;
};
